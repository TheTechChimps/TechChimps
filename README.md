...
- Vercel Cron runs `/api/cron/daily-maintenance` once daily. It requires `CRON_SECRET`, runs the Stripe/live-chat self-healing sweep, and keeps only `latest` and `previous` backup snapshots to avoid storage growing forever.
